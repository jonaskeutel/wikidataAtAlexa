                    ...
                    if (entity.hasOwnProperty('claims')) {
                        for (var j in entity.claims) {
                            ...
                            for (var k in claim) {
                                ...
                                //Get reference(s) of snak
                                if (snak.hasOwnProperty('references')) {
                                    //console.log(snak.references[0]);
                                    for (var i = 0; i < snak.references[0]['snaks-order'].length; i++) {
                                        var refProp = snak.references[0]['snaks-order'][i];
                                        //var refVal = snak.references[0].snaks['P248'][0].datavalue.value;
                                        for (var j = 0; j < snak.references[0].snaks[refProp].length; j++) {
                                            var refVal = snak.references[0].snaks[refProp][j].datavalue.value;

                                            console.log(refVal['entity-type']);
                                            if (refVal['entity-type'] === 'item') {
                                                refVal = 'Q' + refVal['numeric-id'];
                                            } else if (refVal['entity-type'] === 'property') {
                                                refVal = 'P' + refVal['numeric-id'];
                                            } else {
                                                refVal = '';
                                            }
